...
def draw_game_over(score):
    ...
    prompt_txt = font.render("", True, (200, 140, 255))
    screen.blit(prompt_txt, (WIDTH // 2 - prompt_txt.get_width() // 2, HEIGHT // 2 - 30))
    score_txt = font.render(f"Score: {score}/15", True, PURPLE)
    screen.blit(score_txt, (WIDTH // 2 - score_txt.get_width() // 2, HEIGHT // 2 + 20))

def draw_game_over_loading(score, progress, total_steps, congratulations=False):
    # Overlay and loading bar for the 5 second delay
    overlay = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
    overlay.fill((0, 0, 0, 155))
    screen.blit(overlay, (0, 0))
    if congratulations:
        draw_congratulations(score)
        main_txt = "Level Passed!"
    else:
        over_txt = font.render("Game Over!", True, (255, 120, 180))
        screen.blit(over_txt, (WIDTH // 2 - over_txt.get_width() // 2, HEIGHT // 2 - 80))
        main_txt = "Game Over!"
    ...
    bar_y = HEIGHT // 2 + 80
    pygame.draw.rect(screen, (50, 20, 70), (bar_x, bar_y, bar_width, bar_height), border_radius=8)
    fill_width = int(bar_width * (progress / total_steps))
    pygame.draw.rect(screen, CYAN, (bar_x, bar_y, fill_width, bar_height), border_radius=8)
    # Loading text
    loading_font = pygame.font.SysFont("consolas", 22)
    loading_txt = loading_font.render(f"Loading... {int((progress/total_steps)*100)}%", True, (200, 140, 255))
    screen.blit(loading_txt, (WIDTH // 2 - loading_txt.get_width() // 2, bar_y + bar_height + 10))
    # Next prompt
    prompt_font = pygame.font.SysFont("consolas", 22)
    prompt_text = prompt_font.render("", True, (200, 140, 255))
    screen.blit(prompt_text, (WIDTH // 2 - prompt_text.get_width() // 2, bar_y + bar_height + 45))
...
def show_game_over_screen(score, food, food_style, frame_count, congratulations=False):
    # Show loading bar for 5 seconds, then show 'Press any key to continue'
    total_steps = 90  # ~1.5s for bar + 3.5s for prompt = 5s total
    progress = 0
    loading_done = False
    bar_steps = 45  # 1.5s at 30 FPS
    prompt_steps = total_steps - bar_steps
    bar_done = False
    frame = frame_count
    while True:
        clock.tick(30)
        ...
        food_style(food, frame)
        draw_snake([], (0,0,0), (0,0,0)) # Draw nothing, but keep call for consistency
        if not loading_done:
            if progress < bar_steps:
                draw_game_over_loading(score, progress, bar_steps, congratulations=congratulations)
                progress += 1
            else:
                loading_done = True
                prompt_progress = 0
        else:
            draw_game_over_loading(score, bar_steps, bar_steps, congratulations=congratulations)
            prompt_progress += 1
            # Now accept key press to continue, but if prompt_progress < prompt_steps, don't break
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    pygame.quit()
                    sys.exit()
                elif event.type == pygame.KEYDOWN or (event.type == pygame.MOUSEBUTTONDOWN and event.button == 1):
                    if prompt_progress >= prompt_steps:
                        return
            if prompt_progress >= prompt_steps:
                # Show flashing prompt
                if (prompt_progress // 10) % 2 == 0:
                    prompt_font = pygame.font.SysFont("consolas", 22)
                    prompt_text = prompt_font.render("Press any key to continue", True, (200, 140, 255))
                    screen.blit(prompt_text, (WIDTH // 2 - prompt_text.get_width() // 2, (HEIGHT // 2) + 130))
        pygame.display.flip()
        if not loading_done:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
...
def main():
    while True:
        ...
        while True:
            ...
            # Instead of instant key-wait, show loading bar and then prompt
            show_game_over_screen(score, food, food_style, frame_count, congratulations=congratulations)
            break
